      ...
      showDialog(
        ...
        builder: (context) {
          return AlertDialog(
            ...
            content: const Text(
              'Camera access is required to scan images. Please enable it in App Settings.',
            ),
            actions: [
              TextButton(
    ...
    try {
      ...
      if (prediction.detections.isNotEmpty) {
        ...
        if (imageDecoded != null) {
          final int maxSide = max(imageDecoded.width, imageDecoded.height);
          final double scale = maxSide > 1600 ? 1600.0 / maxSide : 1.0;
          final img.Image baseImage = scale < 1
              ? img.copyResize(
                  imageDecoded,
                  width: (imageDecoded.width * scale).round(),
                  height: (imageDecoded.height * scale).round(),
      ...
      return Container(
        ...
        child: Column(
          children: [
            ...
            Text(
              "Please capture an image containing silkworm larvae to generate health analysis and recommendations.",
